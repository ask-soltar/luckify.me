"""
SYSTEM 3b Testing: Round Type × Western Moon (8-cat) × Life Path × Condition
Tests Western lunar cycle + personal destiny signals
"""
...
# Check available columns for western moon and life path
...
if not moonwest_col or not life_col:
    print("ERROR: Could not find moonwest or life_path columns")
    available = [c for c in df.columns if any(x in c.lower() for x in ['moon', 'life'])]
    print(f"Available columns: {available}")
    exit(1)

df = df.dropna(subset=['off_par', 'adj_his_par', 'condition', 'round_type', moonwest_col, life_col])

print("="*130)
print(f"SYSTEM 3b: Round Type × {moonwest_col} × {life_col} × Condition")
print("="*130)
...
if len(positive) > 0:
    ...
    print("-"*130)
    print(f"{'ROI':>6} {'N':>4} {'Edge':>6} {'Conf':>12} {'Round':<12} {'MoonWest':>10} {'Life Path':>10} {'Condition':<10}")
    print("-"*130)

    for _, r in positive.head(25).iterrows():
        print(f"{r['roi']:>5.1f}% {r['n']:>4} {r['edge']:>5.1f}% {r['conf']:>12} {r['round_type']:<12} {str(r['moonwest']):>10} {str(r['life_path']):>10} {r['condition']:<10}")
    ...
    df_combos_sorted.to_csv('system3b_moon_western_lifepath_ALL_combos.csv', index=False)
    print(f"\n[OK] Results saved to: system3b_moon_western_lifepath_ALL_combos.csv")

    # Save positive-only for comparison
    positive.to_csv('system3b_moon_western_lifepath_positive_only.csv', index=False)
    ...
    for _, r in top.iterrows():
        print(f"{r['roi']:>5.1f}% ROI | {r['edge']:>5.1f}% edge | {r['round_type']:<12} × MoonWest {str(r['moonwest']):>3} × Life Path {str(r['life_path']):>3} ({r['condition']:<10}) | N={r['n']:>3}")
