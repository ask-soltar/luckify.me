"""
SYSTEM: Round Type × Gap × Vedic Moon (Tithi, 1-30) × Condition
Tests momentum + lunar day interaction
"""
...
if not tithi_col:
    print("ERROR: Could not find tithi column")
    available = [c for c in df.columns if 'tithi' in c.lower()]
    print(f"Available: {available}")
    exit(1)

df = df.dropna(subset=['off_par', 'adj_his_par', 'condition', 'round_type', 'gap', tithi_col])

print("="*130)
print(f"SYSTEM: Round Type × Gap × {tithi_col} × Condition")
print("="*130)
...
if len(positive) > 0:
    ...
    print("-"*130)
    print(f"{'ROI':>6} {'N':>4} {'Edge':>6} {'Conf':>12} {'Round':<12} {'Gap':<10} {'Tithi':>6} {'Condition':<10}")
    print("-"*130)

    for _, r in positive.head(25).iterrows():
        print(f"{r['roi']:>5.1f}% {r['n']:>4} {r['edge']:>5.1f}% {r['conf']:>12} {r['round_type']:<12} {r['gap']:<10} {str(r['tithi']):>6} {r['condition']:<10}")

    # Save ALL combos
    df_combos_sorted = df_combos.reindex(df_combos['roi'].abs().sort_values(ascending=False).index)
    df_combos_sorted.to_csv('system_gap_vedic_moon_ALL_combos.csv', index=False)
    print(f"\n[OK] Results saved to: system_gap_vedic_moon_ALL_combos.csv")

    # Save positive-only
    positive.to_csv('system_gap_vedic_moon_positive_only.csv', index=False)
