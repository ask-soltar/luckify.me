"""
Matchup Screener — Exec + Upside + Color Model
Scores: (Exec ROI + Upside ROI) / 2 per player
Input: CSV with player exec/upside values and color
Output: Matchups ranked by gap
"""
...
try:
    ...
    print(f"  ERROR: {e}")
    print("  Run: python3.13 build_color_exec_upside_roi_tables.py first")
    exit(1)

# Load matchup CSV
if len(sys.argv) < 2:
    print("\nUsage: python matchup_screener_exec_upside_color.py <matchup_file.csv>")
...
# Score each matchup
print("\n" + "="*140)
print("SCORING MATCHUPS (Exec + Upside + Color Model)")
print("="*140 + "\n")
...
# Save results
output_file = matchup_file.replace('.csv', '_scored_exec_upside.csv')
...
print("\n✓ Model: Exec + Upside + Color (averaged)")
print("✓ Thresholds: ±8% for BET, <±8% SKIP")
