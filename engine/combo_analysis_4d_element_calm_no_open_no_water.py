#!/usr/bin/env python3
"""
4D Analysis: CALM + NO OPEN + NO WATER
Condition: Calm | Round Type (Survival, Positioning, Closing) × Color × Element (Fire, Earth, Wood, Metal)
Excludes: Open, Mixed, REMOVE, Water element
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D (Filtered): Calm + No Open + No Water")
    ...
    # Base CTE - CALM, no Open, no Water
    BASE_CTE = f"""
        base AS (
            SELECT *
            FROM analysis
            WHERE diff_course_avg IS NOT NULL
              AND condition = 'Calm'
              AND round_type IN ('Survival', 'Positioning', 'Closing')
              AND color IS NOT NULL AND color != ''
              AND element IN ('Fire', 'Earth', 'Wood', 'Metal'){year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY round_type, color, element
        )
        SELECT
            'Calm' AS condition,
            c.round_type,
        ...
        FROM by_combo c
        JOIN population p ON c.round_type = p.round_type
        WHERE c.n >= {MIN_N}
        ORDER BY c.round_type, c.color, c.element
    """
    ...
    # Print summary
    print(f"\n{'='*115}")
    print("4D ANALYSIS (FILTERED) - CALM + NO OPEN + NO WATER")
    print("Round Types: Survival, Positioning, Closing | Elements: Fire, Earth, Wood, Metal")
    print("="*115)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['round_type']:12} {row['color']:8} {row['element']:8} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_element_calm_no_open_no_water{year_suffix}.csv")
