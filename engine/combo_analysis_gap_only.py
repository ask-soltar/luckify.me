#!/usr/bin/env python3
"""
Combo Analysis: Gap Only - Color x Condition x Gap

Tests if gap is a standalone signal without exec/upside complexity.
Helps determine if gap dimension is primary or derivative.
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Combo analysis: gap only (3D)")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY color, condition, gap_bucket
        )
        SELECT
            c.color,
            c.condition,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.color, c.condition, c.gap_bucket
    """
    ...
    # Print summary
    print(f"\n{'='*75}")
    print("COMBO ANALYSIS - GAP ONLY (3D: Color x Condition x Gap)")
    print("="*75)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['color']:>8} | {row['condition']:<8} | "
              f"Gap {row['gap_bucket']:>4.0f} | N {row['n']:>4.0f} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"combo_gap_only{year_suffix}.csv")
