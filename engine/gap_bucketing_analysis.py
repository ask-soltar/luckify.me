#!/usr/bin/env python3
"""
Optimal Gap Bucketing Analysis

Tests bucket sizes (25, 50, 75) for gap = exec_bucket - upside_bucket.
Scores each by actionable edges (ratio > 1.2 or < 0.8) + stability.

Gap range: -100 to +100
...
BUCKET_SIZES = [10, 15, 20, 25, 30, 40, 50, 75]
...
def main():
    ...
    print(f"  Gap median: {df['gap'].median():.1f}")
    print(f"\nTesting TWO approaches:")
    print("  1. SIGNED gap (direction matters): -75, -50, -25, 0, +25, +50, +75")
    print("  2. ABSOLUTE gap (magnitude only): 0, 25, 50, 75 (treating -50 and +50 as same)")

    for gap_approach in ["signed", "absolute"]:
        print(f"\n{'='*75}")
        print(f"TESTING {gap_approach.upper()} GAP")
        print(f"{'='*75}")

        results = []

        for bucket_size in BUCKET_SIZES:
            print(f"  Analyzing bucket size {bucket_size} …")

            # Choose bucketing approach
            if gap_approach == "signed":
                gap_bucket_formula = f"""CASE
                        WHEN gap >= 0 THEN (CAST(gap / {bucket_size} AS INTEGER)) * {bucket_size}
                        WHEN gap < 0 THEN -((CAST(ABS(gap) / {bucket_size} AS INTEGER)) * {bucket_size})
                    END"""
            else:  # absolute
                gap_bucket_formula = f"(CAST(ABS(gap) / {bucket_size} AS INTEGER)) * {bucket_size}"
            ...
            bucketed AS (
                SELECT
                    color,
                    condition,
                    {gap_bucket_formula} AS gap_bucket,
                    COUNT(*) AS n,
                ...
                FROM base
                GROUP BY gap_bucket, color, condition
            ),
            population AS (
                SELECT
                    condition,
            ...
            WHERE b.n >= {MIN_N}
            ORDER BY b.color, b.condition, b.gap_bucket
        """
            ...
            print(f"    Actionable edges: {n_actionable} ({100*n_actionable/n_buckets if n_buckets > 0 else 0:.1f}%)")
            print(f"    Avg good_edge: {avg_edge:.2f}%, std: {std_edge:.2f}%")

            # Save CSV
            os.makedirs(OUTPUT_DIR, exist_ok=True)
            bucket_df.to_csv(
                os.path.join(OUTPUT_DIR, f"gap_bucketing_{gap_approach}_{bucket_size}.csv"),
                index=False
            )
        ...
        print(f"\nSUMMARY: {gap_approach.upper()} GAP")
    ...
    print("Files: gap_bucketing_signed_*.csv and gap_bucketing_absolute_*.csv")
    print("Compare to see if direction (signed) or magnitude (absolute) matters more")
