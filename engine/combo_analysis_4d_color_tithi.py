#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Color x Tithi (Vedic Lunar Day)

Color shifts every 24 hours (feng shui environmental energy).
Tithi is Vedic lunar day (10 categories in this system).
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Color x Tithi")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, moon
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.moon
    """
    ...
    # Print summary
    print(f"\n{'='*120}")
    print("4D ANALYSIS - Condition x Round Type x Color x Tithi (Vedic Lunar Day)")
    print("="*120)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} {str(row['color']):<10} {str(row['moon']):<20} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_color_tithi{year_suffix}.csv")
