#!/usr/bin/env python3
"""
Combo Analysis with Gap - Color x Exec x Upside x Condition x Gap x Round Type

Finds winning combinations across all 6 dimensions.
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Combo analysis with gap dimension")
    ...
    df["gap_bucket"] = pd.to_numeric(df["gap_bucket"], errors="coerce")
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND gap_bucket IS NOT NULL{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            SELECT
                ...
                CAST(gap_bucket AS INTEGER) AS gap_bucket,
                COUNT(*) AS n,
            ...
            FROM base
            GROUP BY color, condition, exec_bucket, upside_bucket, gap_bucket
        )
        SELECT
            c.color,
            c.condition,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.color, c.condition, c.exec_bucket, c.upside_bucket, c.gap_bucket
    """
    ...
    # Print summary
    print(f"\n{'='*75}")
    print("COMBO ANALYSIS WITH GAP - SUMMARY")
    print("="*75)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['color']:>8} | {row['condition']:<8} | "
              f"Exec {row['exec_bucket']:>3.0f} | Up {row['upside_bucket']:>3.0f} | "
              f"Gap {row['gap_bucket']:>3.0f} | N {row['n']:>4.0f} | "
              f"Edge {row['good_edge']:>+6.1f}% | Ratio {row['ratio']:>6.2f}")
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"combo_with_gap{year_suffix}.csv")
