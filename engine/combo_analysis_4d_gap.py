#!/usr/bin/env python3
"""
4D Analysis with Gap: Round Type x Condition x Color x Gap

Tests if gap dimension adds signal to foundation.
Gap = exec_bucket - upside_bucket (signed, direction matters).
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="4D: Round Type x Condition x Color x Gap")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY round_type, condition, color, gap_bucket
        )
        SELECT
            c.round_type,
            c.condition,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.round_type, c.condition, c.color, c.gap_bucket
    """
    ...
    # Print summary
    print(f"\n{'='*90}")
    print("4D ANALYSIS - Round Type x Condition x Color x Gap")
    print("="*90)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_gap{year_suffix}.csv")
