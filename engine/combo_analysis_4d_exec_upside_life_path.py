#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec_Upside x Life Path

Exec_Upside are quartile buckets (0, 25, 50, 75).
Life Path has 9 values (1-9, numerology).
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec_Upside x Life Path")
    ...
    df["life_path"] = pd.to_numeric(df["life_path"], errors="coerce")
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND life_path IS NOT NULL{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket, life_path
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket, c.life_path
    """
    ...
    # Print summary
    print(f"\n{'='*115}")
    print("4D ANALYSIS - Condition x Round Type x Exec_Upside x Life Path")
    print("="*115)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec{int(row['exec_bucket']):>2} Upside{int(row['upside_bucket']):>2} LP{int(row['life_path']):>1} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside_life_path{year_suffix}.csv")
