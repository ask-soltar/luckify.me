#!/usr/bin/env python3
"""
3D Foundation Analysis: Round Type x Condition x Color

Core analysis with must-have dimensions only.
Tests if color has predictive power within round type and condition context.
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="3D Foundation: Round Type x Condition x Color")
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND round_type IS NOT NULL AND round_type != ''{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY round_type, condition, color
        )
        SELECT
            c.round_type,
            c.condition,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.round_type, c.condition, c.color
    """
    ...
    # Print summary
    print(f"\n{'='*85}")
    print("3D FOUNDATION ANALYSIS - Round Type x Condition x Color")
    print("="*85)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"3d_foundation{year_suffix}.csv")
