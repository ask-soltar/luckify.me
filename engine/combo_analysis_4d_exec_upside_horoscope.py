#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec_Upside x Horoscope (Western Zodiac)

Exec_Upside are quartile buckets (0, 25, 50, 75).
Horoscope has 12 zodiac signs (Aries, Taurus, ... Pisces).
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec_Upside x Horoscope")
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND horoscope IS NOT NULL AND horoscope != ''{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket, horoscope
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket, c.horoscope
    """
    ...
    # Print summary
    print(f"\n{'='*125}")
    print("4D ANALYSIS - Condition x Round Type x Exec_Upside x Horoscope (Western Zodiac)")
    print("="*125)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec{int(row['exec_bucket']):>2} Upside{int(row['upside_bucket']):>2} {str(row['horoscope']):<15} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside_horoscope{year_suffix}.csv")
