#!/usr/bin/env python3
"""
4D Analysis with Horoscope: Condition x Round Type x Color x Horoscope

Tests if Horoscope (12 zodiac signs) adds predictive value.
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Color x Horoscope")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, horoscope
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.horoscope
    """
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("4D ANALYSIS - Condition x Round Type x Color x Horoscope")
    print("="*100)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_horoscope{year_suffix}.csv")
