#!/usr/bin/env python3
"""
5D Analysis with Element + Gap: Condition x Round Type x Color x Element x Gap

Tests if Element + Gap together add predictive value beyond Element alone.
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="5D: Condition x Round Type x Color x Element x Gap")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, element, gap_bucket
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.element, c.gap_bucket
    """
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("5D ANALYSIS - Condition x Round Type x Color x Element x Gap")
    print("="*100)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<8} {row['round_type']:12} {row['color']:>8} | "
              f"Elem {row['element']:<8} Gap {row['gap_bucket']:>+4.0f} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"5d_element_gap{year_suffix}.csv")
