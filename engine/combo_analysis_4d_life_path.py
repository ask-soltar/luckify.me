#!/usr/bin/env python3
"""
4D Analysis with Life Path: Condition x Round Type x Color x Life Path

Tests if Life Path adds predictive value.
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Color x Life Path")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, life_path
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.life_path
    """
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("4D ANALYSIS - Condition x Round Type x Color x Life Path")
    print("="*100)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_life_path{year_suffix}.csv")
