#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec_Upside x Chinese Zodiac

Exec_Upside are quartile buckets (0, 25, 50, 75).
Chinese Zodiac has 12 animals (Rat, Ox, Tiger, ... Pig).
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec_Upside x Chinese Zodiac")
    ...
    df["chinese_zodiac"] = df["chinese_zodiac"].astype(str).str.strip()
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND chinese_zodiac IS NOT NULL AND chinese_zodiac != ''{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket, chinese_zodiac
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket, c.chinese_zodiac
    """
    ...
    # Print summary
    print(f"\n{'='*130}")
    print("4D ANALYSIS - Condition x Round Type x Exec_Upside x Chinese Zodiac")
    print("="*130)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec{int(row['exec_bucket']):>2} Upside{int(row['upside_bucket']):>2} {str(row['chinese_zodiac']):<12} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside_chinese_zodiac{year_suffix}.csv")
