#!/usr/bin/env python3
"""
4D Analysis: CALM + NO OPEN
Condition: Calm | Round Type (Survival, Positioning, Closing) × Color × Element
Excludes: Open, Mixed, REMOVE
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D (Filtered): Calm + No Open")
    ...
    # Base CTE - CALM ONLY, exclude Open
    BASE_CTE = f"""
        base AS (
            SELECT *
            FROM analysis
    ...
    # Print summary
    print(f"\n{'='*110}")
    print("4D ANALYSIS (FILTERED) - CALM + NO OPEN (Survival, Positioning, Closing only)")
    print("="*110)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_element_calm_no_open{year_suffix}.csv")
