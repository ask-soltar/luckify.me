#!/usr/bin/env python3
"""
4D Analysis with Element: Condition x Round Type x Color x Element

Tests if Wu Xing Element adds predictive value.
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Color x Element")
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND element IS NOT NULL AND element != ''{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, element
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.element
    """
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("4D ANALYSIS - Condition x Round Type x Color x Element")
    print("="*100)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_element{year_suffix}.csv")
