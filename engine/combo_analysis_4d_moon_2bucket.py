#!/usr/bin/env python3
"""
4D Analysis with Moon (2-Bucket: Shukla vs Krishna)

Shukla Paksha (Waxing/Bright): New → Full
Krishna Paksha (Waning/Dark): Full → New

Tests if reduced moon dimensionality improves transfer rate vs 8-phase moon (0%)
"""
...
def get_moon_paksha(tithi):
    """Extract Shukla/Krishna from Tithi (e.g., 'Shukla Jaya' -> 'Shukla')"""
    if pd.isna(tithi):
        return None
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Color x Moon(2-bucket)")
    ...
    # Convert 8-phase moon to 2-bucket Shukla/Krishna
    df["moon_phase"] = df["moon"].astype(str).str.strip()
    df["moon_2bucket"] = df["moon_phase"].apply(get_moon_paksha)
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND moon_2bucket IS NOT NULL{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, moon_2bucket
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.moon_2bucket
    """
    ...
    # Print summary
    print(f"\n{'='*100}")
    print("4D ANALYSIS - Condition x Round Type x Color x Moon (2-bucket: Shukla/Krishna)")
    print("="*100)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<8} {row['round_type']:12} {row['color']:>8} | "
              f"Moon {str(row['moon_2bucket']):<10} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_moon_2bucket{year_suffix}.csv")
