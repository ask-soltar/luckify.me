#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec Bucket x Upside Bucket

Tests if player-specific signals (Exec+Upside) add predictive value.
Exec/Upside are quartile buckets (0, 25, 50, 75).
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec Bucket x Upside Bucket")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket
    """
    ...
    # Print summary
    print(f"\n{'='*110}")
    print("4D ANALYSIS - Condition x Round Type x Exec Bucket x Upside Bucket")
    print("="*110)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec {int(row['exec_bucket']):>2} Upside {int(row['upside_bucket']):>2} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside{year_suffix}.csv")
