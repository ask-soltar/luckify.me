#!/usr/bin/env python3
"""
Add Wu Xing Element column to ANALYSIS_v2.

Sources from Golf_Analytics column BC (Wu Xing Element).
...
def main():
    ...
    # Extract element from Golf_Analytics
    if "Wu Xing Element" in golf_df.columns:
        element_data = golf_df[["Player", "Venue", "Year", "Wu Xing Element"]].copy()
        element_data.columns = ["player_name", "event_name", "year", "element"]

        print(f"\nMerging Wu Xing Element by player + event + year ...")
        ...
        matched = analysis_merged["element"].notna().sum()
        print(f"  Matched {matched} / {len(analysis_merged)} rows")

        # Save to CSV for inspection
        csv_path = os.path.join(BASE_DIR, "analysis_v2_with_element.csv")
        analysis_merged.to_csv(csv_path, index=False)
        print(f"  Saved to {csv_path}")

        # Push to sheet
        print(f"\nAdding 'element' column to {ANALYSIS_SHEET} ...")
        ws = ss.worksheet(ANALYSIS_SHEET)

        current_cols = ws.col_count
        if current_cols < 25:
            ws.add_cols(25 - current_cols)
            print(f"  Expanded to 25 columns")
            time.sleep(0.5)

        # Update header
        ws.update(range_name="Y1", values=[["element"]], value_input_option="USER_ENTERED")
        time.sleep(0.5)

        # Update data in batches
        batch_size = 500
        for i in range(0, len(analysis_merged), batch_size):
            batch = analysis_merged.iloc[i:min(i+batch_size, len(analysis_merged))]["element"].values
            batch_data = [[val if pd.notna(val) else ""] for val in batch]

            start_row = 2 + i
            end_row = start_row + len(batch) - 1

            range_str = f"Y{start_row}:Y{end_row}"
        ...
        print(f"\nDone! Added 'element' column to {ANALYSIS_SHEET}")
        print(f"Total rows with element data: {matched}")
        print(f"\nColumn Y now contains Wu Xing Elements (Wood, Fire, Earth, Metal, Water)")

    else:
        print("\nERROR: 'Wu Xing Element' column not found in Golf_Analytics")
