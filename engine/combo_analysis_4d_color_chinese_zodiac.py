#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Color x Chinese Zodiac

Color shifts every 24 hours (feng shui environmental energy).
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Color x Chinese Zodiac")
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, color, chinese_zodiac
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.color, c.chinese_zodiac
    """
    ...
    # Print summary
    print(f"\n{'='*125}")
    print("4D ANALYSIS - Condition x Round Type x Color x Chinese Zodiac")
    print("="*125)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} {str(row['color']):<10} {str(row['chinese_zodiac']):<12} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_color_chinese_zodiac{year_suffix}.csv")
