#!/usr/bin/env python3
"""
Combo Analysis — Color × Exec Bucket × Upside Bucket × Condition

Finds winning combinations with stable positive edges.
...
Metrics per bucket:
  ...
  - ratio      : (good/bad) for this combo vs population
                 > 1.2 = actionable positive edge
                 < 0.8 = actionable negative edge

Filters:
  - condition IN ('Calm', 'Moderate', 'Tough')
  - N >= 30 minimum sample
  - diff_course_avg not null
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Combo analysis with optional year filtering")
    ...
    query = f"""
        ...
        FROM by_combo c
        JOIN population p ON c.condition = p.condition
        WHERE c.n >= {MIN_N}
        ORDER BY c.color, c.condition, c.exec_bucket, c.upside_bucket
    """
    ...
    print(f"Ratio > 1.2 = more good/bad than population baseline\n")
    ...
    # Top positive combos
    print(f"\nTop 10 Positive Edges (by good_edge):")
    print("-" * 75)
    top_positive = combo_df[combo_df["ratio"] > 1.0].nlargest(10, "good_edge")
    for _, row in top_positive.iterrows():
        print(f"  {row['color']:>8} | Cond: {row['condition']:<8} | "
              f"Exec: {row['exec_bucket']:>3.0f}–{row['exec_bucket']+25:>3.0f} | "
              f"Up: {row['upside_bucket']:>3.0f}–{row['upside_bucket']+25:>3.0f} | "
              f"N: {row['n']:>4.0f} | "
              f"Edge: {row['good_edge']:>+6.1f}% | "
              f"Ratio: {row['ratio']:>6.2f}")

    # Top negative combos
    print(f"\nTop 10 Negative Edges (by good_edge):")
    print("-" * 75)
    top_negative = combo_df[combo_df["ratio"] < 1.0].nsmallest(10, "good_edge")
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"combo_analysis{year_suffix}.csv")
