#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec_Upside x Moon (10 Tithi values)

Exec_Upside are quartile buckets (0, 25, 50, 75).
Moon uses all 10 Tithi values: Shukla/Krishna x Jaya/Purna/Nanda/Rikta/Bhadra
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec_Upside x Moon (10 Tithi)")
    ...
    df["moon"] = df["moon"].astype(str).str.strip()
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket, moon
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket, c.moon
    """
    ...
    # Print summary
    print(f"\n{'='*120}")
    print("4D ANALYSIS - Condition x Round Type x Exec_Upside x Moon (10 Tithi values)")
    print("="*120)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec{int(row['exec_bucket']):>2} Upside{int(row['upside_bucket']):>2} {str(row['moon']):<15} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside_moon10tithi{year_suffix}.csv")
