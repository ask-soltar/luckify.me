#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec_Upside x Western Moon (8 phases)

Exec_Upside are quartile buckets (0, 25, 50, 75).
Western Moon has 8 phases (New Moon, Waxing Crescent, First Quarter, etc.)
"""
...
def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec_Upside x Western Moon (8 phases)")
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND moonwest IS NOT NULL AND moonwest != ''{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket, moonwest
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket, c.moonwest
    """
    ...
    # Print summary
    print(f"\n{'='*120}")
    print("4D ANALYSIS - Condition x Round Type x Exec_Upside x Western Moon (8 phases)")
    print("="*120)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec{int(row['exec_bucket']):>2} Upside{int(row['upside_bucket']):>2} {str(row['moonwest']):<20} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside_moonwest{year_suffix}.csv")
