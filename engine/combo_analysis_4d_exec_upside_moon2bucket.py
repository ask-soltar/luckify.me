#!/usr/bin/env python3
"""
4D Analysis: Condition x Round Type x Exec_Upside x Moon (2-bucket: Shukla/Krishna)

Exec_Upside are quartile buckets (0, 25, 50, 75).
Moon is reduced to 2 buckets (Shukla=Waxing, Krishna=Waning).
"""
...
def get_moon_paksha(tithi):
    """Extract Shukla/Krishna from Tithi"""
    if pd.isna(tithi):
        return None
    tithi_str = str(tithi).strip()
    if tithi_str.startswith("Shukla"):
        return "Shukla"
    elif tithi_str.startswith("Krishna"):
        return "Krishna"
    return None

def main():
    parser = argparse.ArgumentParser(description="4D: Condition x Round Type x Exec_Upside x Moon (2-bucket)")
    ...
    # Extract 2-bucket moon
    df["moon_paksha"] = df["moon"].astype(str).apply(get_moon_paksha)
    ...
    BASE_CTE = f"""
        base AS (
            ...
            WHERE diff_course_avg IS NOT NULL
              ...
              AND moon_paksha IS NOT NULL{year_filter}
        )
    """
    ...
    query = f"""
        ...
        by_combo AS (
            ...
            FROM base
            GROUP BY condition, round_type, exec_bucket, upside_bucket, moon_paksha
        )
        SELECT
            c.condition,
            c.round_type,
        ...
        FROM by_combo c
        JOIN population p ON c.condition = p.condition AND c.round_type = p.round_type
        WHERE c.n >= {MIN_N}
        ORDER BY c.condition, c.round_type, c.exec_bucket, c.upside_bucket, c.moon_paksha
    """
    ...
    # Print summary
    print(f"\n{'='*115}")
    print("4D ANALYSIS - Condition x Round Type x Exec_Upside x Moon (2-bucket: Shukla/Krishna)")
    print("="*115)
    ...
    for _, row in top_positive.iterrows():
        print(f"  {row['condition']:<10} {row['round_type']:12} Exec{int(row['exec_bucket']):>2} Upside{int(row['upside_bucket']):>2} Moon {str(row['moon_paksha']):<8} | "
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"4d_exec_upside_moon2bucket{year_suffix}.csv")
